...
6. On confirmation: **before composing the merge command, re-read
   ...
   branch stranded on `origin` (issue #13). `forge:pr-merge <pr>` — subject `<title>
   (#<pr>)`, body "No semantic content (ADR-001): <one line>." Then return the checkout
   to a clean, current `main` and remove the merged branch, exactly as `/t-ship` does:
