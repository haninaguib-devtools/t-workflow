...
**6.5 Driving an initiative — or a single task** ([ADR-004](adr/004-autonomous-initiative-driving.md),
[ADR-006](adr/006-single-task-driving.md)) is the opt-in alternative to working each stage by
hand: `/t-drive <initiative-id>` chains `/t-plan`+`/t-work`+`/t-review` across an
initiative's children on one integration branch — the alternative to 6.1's
one-PR-per-child default — merging each child into it once that child's own review
authorizes the merge, excluding — never auto-cancelling — a child that still fails after
one bounded retry, then stopping once for the human's single confirmation on the
initiative's combined PR to `main`. Handed a plain task instead (ADR-006), the same command
drives that one task's own ordinary pipeline — plan and review only where the
protected-surface gates require them, one bounded retry, no integration branch and no
autonomous merge — chained into `/t-ship`'s merge-confirmation gate, whose pause is the
same single stop; a solo drive is not work larger than one PR, and lives here only because
it shares the exception. `/t-drive` is the one explicitly-invoked exception to §5's
"nothing chains" rule (ADR-001 D1), in both modes; every other stage still stops and names
the next command.
