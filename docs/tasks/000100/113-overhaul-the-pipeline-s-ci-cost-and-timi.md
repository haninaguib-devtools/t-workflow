...
- **Two defects found at this task's own ship attempt (2026-08-30), fixed in this same
  change before landing.** (1) The new `/t-ship` step 5 said to run
  `github-bootstrap.sh` to flip the live required-check contexts *pre-merge* — but the
  script's own gating looks for the new `checks` context on `main`, which cannot exist
  until the merge lands, so pre-merge it fell through to its fallback. Step 5 now does
  a direct API PATCH to the exact new list and forbids running the script pre-merge;
  the script stays a post-merge true-up only. (2) That fallback itself *overwrote* live
  protection with `null`, silently disabling required checks on an already-protected
  repo — observed live on this repo during the ship attempt (protection was restored by
  hand minutes later). The fallback now reads the currently-enforced value first and
  re-asserts it unchanged when the context can't yet be confirmed on `main`.
- **Landed by hand, outside the pipeline, at the owner's explicit direction
  (2026-08-30).** After the ship attempt above surfaced the two defects mid-gate, the
  owner chose to finish by direct push to `main` rather than a fix-pass/re-review/
  re-ship cycle: PR #115 was closed unmerged and its branch content (plus the two fixes
  above) pushed as one squash commit, with branch protection's admin enforcement lifted
  by the owner for the push and restored immediately after. The cold review that ran on
  PR #115 covered everything except the two fix hunks above.
