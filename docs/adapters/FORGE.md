...
`delete_branch_on_merge` setting (asserted by `scripts/github-bootstrap.sh`) deletes the
...
they were. A stale local worktree or branch, when one is actually in the way, is
`/t-clean`'s job (ADR-002), never a side effect of merging.
...
`forge:pr-find-by-task`); all-state PRs repo-wide, every head branch at once (id, state,
head branch — `/t-clean` uses this to cross-reference every local `wip/*`/`fix/*` branch
against its PR's state in one round trip, the same shape `forge:pr-find-by-task` filters
to a single task).
